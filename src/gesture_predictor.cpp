...
{
// Serial.printf("size output: %d\n", sizeof(output) / sizeof(float));
#if DEBUG_INF_RES
  Serial.println("");
#endif
  ...
  for (int i = 0; i < kGestureCount; ++i)
  {
    ...
    Serial.printf("%s: %f", LABELS[i].c_str(), output[i]);
    int barNum = static_cast<int>(roundf(output[i] * 10)); // output配列のi番目に格納された数字(float型)を10倍して格納
